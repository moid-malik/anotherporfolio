...
const Page1 = () => {
  ...
  const mouseMoveHandle = (e: MouseEvent<HTMLDivElement>) => {
    if (tiltTextRef.current) {
      const rect = tiltTextRef.current.getBoundingClientRect();
      setXVal((e.clientX - rect.x - rect.width) / 12);
      setYVal(-(e.clientY - rect.y - rect.height) / 5);
    }
  };
